...
<head>
    <title>ALL  - ФАРА-АВТО Интернет магазин></title>
    <meta charset="UTF-8">
    <!-- Bootstrap Core CSS -->
            ...
            <div class="row">
                ...
                <c:forEach items="${products}" var="item">
                    <div class='col-sm-6 col-lg-6 col-md-6'>
                        <div class='thumbnail'>
                            <a href='/product/<c:out value="${item.url}" />'><img src='/resources/img/<c:out value="${item.images.get(0).name}" />'></a>
                            <div class='caption'>
                                <h4 class='price'><span><c:out value="${item.price}" /></span> грн</h4>
                                <h4><a href='/product/<c:out value="${item.url}" />'><c:out value="${item.name}" /></a></h4>
                                <p>Производитель: <c:out value="${item.manufacturer.name}" /></p>
                                <a href='/product/<c:out value="${item.url}" />'><button class='btn_small' >Детальнее</button></a>
                            </div>
                        </div>
                    </div>
                </c:forEach>
